...
out.dir = "OM_Null_Modeling/"

bnti.in.path = "MCD_bNTI_Outcomes/"

dendro.in.path = "MCD_Dendrograms/"
...
beta.disp = read.csv("ECA2_FTICR_BetaDisp.csv",stringsAsFactors = F)
head(beta.disp)
str(beta.disp)

# read texture
texture = read.csv("VGC_texture.csv",stringsAsFactors = F)
colnames(texture)[which(colnames(texture) == 'Kit_ID')] = "Site"
head(texture)
str(texture)

# read bnti metrics
bnti = read.csv("MCD_bNTI_Outcomes/MCD_bNTI_and_regress_metrics.csv")
